...
seeds = [{"tf_seed": 343053, "np_seed": 999360}]
...
for name, experiment in exp_specs["experiments"]:
    input_data_file = os.path.join(data_directory, experiment["input_data_file"])
    try:
        print("*" * 30)
        printt("Loading input data")
        input_data = cPickle.load(open(input_data_file,'rb'))
        preProcessData(input_data)
        # Prepare data as per task
        if mode_experiment=="epitope":
            input_data = remove_abs(input_data)
        else:
            input_data = remove_ags(input_data)
        ...
        for i, seed_pair in enumerate(seeds):
            ...
            learning_rate = 0.001
            model = PWClassifier(experiment["layers"], experiment["layer_args"], data["input"], learning_rate, params_preTrain, 0.1, outdir)
            
            printt("*"*30)
            printt("Activations for inputdataset set")
            dir_out = outdir + "/rep_"+str(i)+"/"
            if not os.path.exists(dir_out):
                os.mkdir(dir_out)
            TrainTest(results_processor).activate_for_proteins(data["input"], model, experiment["layers"], dir_out)
